...
function getInfo() {
  ...
  if (props.testLeaveVo && Object.keys(props.testLeaveVo).length) {
    ...
    getLeave(props.modelValue).then(response => {
      ...
      anyNodeList(form.value.instanceId).then(response => {
...
function rejectLast() {
  proxy.$refs["leaveApproveRef"].validate(valid => {
    if (valid) {
      rejectLast(form.value, props.taskId, form.value.message, flowStatus.value).then(response => {
...
function taskBack() {
  proxy.$refs["leaveApproveRef"].validate(valid => {
    if (valid) {
      taskBack(form.value, props.taskId, form.value.message, flowStatus.value).then(response => {
