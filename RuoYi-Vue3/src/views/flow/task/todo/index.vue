<template>
  <div class="app-container">
    ...
    <component :is="approve" v-model="businessId" :type="'0'" :taskId="taskId" :showAnyNode="showAnyNode" :disabled="false" :formId="formPath" @refresh="getList"></component>
...
/** 办理按钮操作 */
function handle(row, _showAnyNode) {
  approve.value = null;
  taskId.value = row.id;
  businessId.value = row.businessId;
  showAnyNode.value = _showAnyNode;
  if (!row.formPath) {
  ...
  if (row.formCustom == 'N') {
    ...
    import(/* @vite-ignore */`../../../../views/form/definition/formDialog.vue`).then((module) => {
      approve.value = module.default;
    });
  }
}
